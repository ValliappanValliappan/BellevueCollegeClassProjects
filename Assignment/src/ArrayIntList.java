...
//Name:Valliappan Valliappan	Assignment #7
//Class:CS 211      Date:November 17, 2017
...
public class ArrayIntList implements Iterable<Integer> {
    ...
    public ArrayIntList upToNowTotal(){
    	ArrayIntList totals = new ArrayIntList(getCapacity());//an ArrayIntList with the same capacity of the original list which will contain
    	int currentTotal=0;//the total that will be added to the element that is returned by get(i)
    	ArrayIntListIterator iterator=(ArrayIntListIterator) iterator();
    	while(iterator.hasNext()){
    		int currentElement=iterator.next();
    		totals.add(currentTotal+currentElement);//currentTotal is added to currentElement of the original list to get the total up to that index. the result is then added as an element to totals
    		currentTotal+=currentElement;//setting currentTotal to the current total up to the currentElement
    	}
    	return totals;
    }
    /**
    ...
    public boolean isPairSorted(){
    	ArrayIntListIterator iterator=(ArrayIntListIterator) iterator();
    	if(!iterator.hasNext()){//if there is 0 elements, iterator.next() will cause an error. This line will prevent that exception
    		return true;
    	}
    	int firstNumberOfPair=iterator.next();//Setting the first element of the list to firstNumberOfPair
    	while(iterator.hasNext()){//loop runs only if there a second element in the pair.
    		int secondNumberInPair=iterator.next();//Setting the second element of the pair of numbers
    		if(secondNumberInPair<firstNumberOfPair){//if the pair is not sorted
    			return false;
    		}
    		if(!iterator.hasNext()){//will be true if there are no more pairs
    			break;//get out of the loop
    		}
    		firstNumberOfPair=iterator.next();//setting the first element of the next pair to firstNumberOfPair
    	}
    	return true;//If the method reaches this part, all the pairs are sorted
    }
    /**
     * Question 3
     */
    public void removeLast(int numberOfElements){//numberOfElements is the number of elements to be removed from the rear of the array
    	int indexToRemoveFrom=size()-numberOfElements;//storing the number because size() returns a different value each time you remove a value from the list
    	for(int i=1;i<=numberOfElements;i++){//to keep track of the number of elements being removed from the back of the array
    		remove(indexToRemoveFrom);//removing elements in the same position as removing shifts the rights side of the list to the left
    	}
    }
    // pre : capacity >= 0
